...
typedef enum
{
  ...
  NWLOG_ERROR = 3,
  NWLOG_FATAL = 4,
  NWLOG_NUM_MLOG_LEVEL // always be the last one for counting purpose
} nwlog_level_e;
...
#define ERROR_DUMP(...) fprintf(stderr, __VA_ARGS__)
  ...
  public:
    int init(const std::string &app_name, bool log_to_file, std::string &log_file_dir, nwlog_level_e min_log_level);
    int flush();
    int flush_unlocked();
    int sync_to_disk();
    int sync_to_disk_unlocked();
    void dump_logging_config();
    bool should_log_on_module_level(const std::string &module, nwlog_level_e level);
    bool should_log_to_file() const { return log_to_file_; }
    void set_log_to_file(bool log_to_file) { log_to_file_ = log_to_file; }
    void set_min_log_level(nwlog_level_e min_log_level) { log_min_level_ = min_log_level; }
    void set_log_file_dir(std::string &log_file_dir) { log_dir_prefix_ = log_file_dir; }
    int write_log(const char *module, nwlog_level_e level, const char *filename, int line, char *body_buf, int body_len);
  ...
  private:
    ...
    nwlog_level_e log_min_level_;
    std::map<std::string, nwlog_level_e> module_level_;
