...
namespace Vins
{
  ...
  nwlog_level_e log_level_string_to_enum(const char *level)
  {
    if (strcmp(level, "VERBOSE") == 0)
    {
      return NWLOG_VERBOSE;
    }
    ...
    return NWLOG_NUM_MLOG_LEVEL;
  }

  int init_logging(const char *app_name, bool log_to_file, std::string log_file_dir, nwlog_level_e min_log_level)
  {
